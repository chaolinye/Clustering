        ...
        implements XYDataset, DomainInfo, RangeInfo
{
    ...
    public SampleXYDataset2(double[][][] data){
        xValues = new Double[data.length][];
        yValues = new Double[data.length][];
        for(int i=0;i<data.length;i++){
            xValues[i]=new Double[data[i].length];
            yValues[i]=new Double[data[i].length];
        }
        ...
        for(int i=0;i<data.length;i++){
            for(int j=0;j<data[i].length;j++){
                double d4 = data[i][j][0];
                xValues[i][j]=new Double(d4);
                if(d4 < d)
                    d = d4;
                if(d4 > d1)
                    d1 = d4;
                double d5 =data[i][j][1];
                yValues[i][j]=new Double(d5);
                if(d5 < d2)
                    d2 = d5;
                if(d5 > d3)
                    d3 = d5;
